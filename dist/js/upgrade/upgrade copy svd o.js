            ...
            plans = [
              {
                ...
                cta: {
                  ...
                  mousedown: () => {
                    this.selectedPlan = 'pro-' + (this.yearly ? 'yearly' : 'monthly');
                    return this.to('edit-payment-info');
                  }
                }
              },
              {
                    ...
                    '.period-monthly ' + _lang['select-plan']['pay-monthly'],
                    {
                      ...
                      events: {
                        mousedown: () => {
                          ...
                          return this.yearly = !this.yearly;
                        }
                      }
                    },
                ...
                desc = [
                  ...
                  [
                    ...
                    p.cta.elDesc ? p.cta.elDesc : {
                      tag: '.cta ' + p.cta.label,
                      events: {
                        mousedown: p.cta.mousedown.bind(this)
                      }
                    },
                    [
                      'ul.features',
...
<div class="bloc address-bloc">
	...
	<div class="triple-bloc">
            ...
            radio = this.create(r.SelectboxPlain,
        {
            ...
            refreshForm = () => {
              if (radio.current.option.id === 'business') {
                $(this.content).find('.bloc1-full-name').hide();
                return $(this.content).find('.bloc1-company').show();
              } else {
                $(this.content).find('.bloc1-full-name').show();
                return $(this.content).find('.bloc1-company').hide();
              }
            };
            radio.on('pick',
        refreshForm);
            ...
            this.content.appendChild(r.toDOM(['.bloc0',
        [['.bloc0-title',
        ['.number 2',
        ...
        '.card-bloc']]));
            appendCardForm = () => {
              var elements;
              $(this.el).find('.card-bloc').append(r.toDOM(`div 
...
<div class="input-bloc">
	...
	<input id="name-on-card" />
              ...
              card = elements.create('card',
        {
              ...
              card.mount($(this.el).find('#card-element').get(0));
              ...
              paymentMethodBloc = this.create(r.PaymentMethodBloc,
        {
                onchange: () => {
                  ...
                  appendCardForm();
                  return $(this.el).find('.card-bloc').append(r.toDOM({
                    tag: '.cancel ' + _lang['edit-payment-info']['payment-method'].cancel,
                    events: {
                      mousedown: () => {
                        $(this.el).find('.card-bloc').html('');
        ...
        'ZW'];
            ...
            refreshVATField = () => {
              var country;
              country = $(this.el).find('#country').val().trim();
              if (!taxList[country]) {
                $(this.el).find('#vat-number').val('');
                $(this.el).find('label[for=vat-number]').hide();
                return $(this.el).find('#vat-number').hide();
              } else {
                $(this.el).find('#vat-number').show();
                return $(this.el).find('label[for=vat-number]').show();
              }
            };
            $(this.el).find('select').get(0).onchange = refreshVATField;
            refreshVATField();
            cta = $(this.el).find('.cta').get(0);
            return cta.addEventListener('pointerdown',
        () => {
              var billing_infos,
        country,
        valid,
        validateForm;
              billing_infos = {};
              if (this.radio.current.option.id === 'business') {
                billing_infos.name = $(this.el).find('#company-name').val().trim();
                if ($(this.el).find('#vat-number').val().trim()) {
                  billing_infos.eu_vat = $(this.el).find('#vat-number').val().trim();
                }
              } else {
                billing_infos.name = $(this.el).find('#full-name').val().trim();
              }
              billing_infos.customer_type = this.radio.current.option.id;
              billing_infos.address = {
                line1: $(this.el).find('#line1').val().trim(),
                // line2 : $(@el).find('#line-2').val()
                postal_code: $(this.el).find('#postal-code').val().trim(),
        ...
        ''));
                ...
                if (taxList[country]) {
                  if (!billing_infos.eu_vat) {
                    $(this.el).find('#errors').html(_lang['edit-payment-info'].errors['vat-cannot-be-blank']);
                    return;
                  }
                  if (!valid(billing_infos.eu_vat)) {
                    $(this.el).find('#errors').html(_lang['edit-payment-info'].errors['invalid-vat-number']);
                    return;
                  }
                }
              }
              if (!billing_infos.name || !billing_infos.address.line1 || !billing_infos.address.postal_code || !billing_infos.address.city) {
                $(this.el).find('#errors').html(_lang['edit-payment-info'].errors['required-fields-missing']);
                return;
              }
              $(cta).addClass('spin');
              validateForm = (data) => {
                ...
                return $.ajax({
                  ...
                  error: () => {
                    ...
                    return $(this.el).find('#errors').html('Internal server error'); //translate #todo
                  },
                  success: (resp) => {
                    $(cta).removeClass('spin');
                    if (!resp.error) {
                      this._a.user.billing_infos = data.billing_infos;
                      return this.to('review-order');
                    } else {
                      return $(this.el).find('#errors').html(resp.error);
                    }
                  }
                });
              };
              //test-only
              //todo
              this._a.user.billing_infos = billing_infos;
              this._a.user.payment_method = {
                card: {
                  brand: 'mastercard',
                  exp_month: 5,
                  exp_year: 2023,
              ...
              if (shouldSendMethod) { //tmp
                return this.stripe.createPaymentMethod({
                  ...
                  if (result.error) {
                    ...
                    this._a.user.payment_method = result.paymentMethod;
                    this._a.user.payment_method.name_on_card = $(this.el).find('#name-on-card').val();
          ...
          cta: {
            ...
            mousedown: function() {
              ...
              return $.ajax({
                ...
                error: () => {
                  $(this.el).find('.cta').removeClass('spin');
                  return $(this.el).find('#errors').html('Internal server error'); //todo #translate
                },
                success: (resp) => {
                  $(this.el).find('.cta').removeClass('spin');
                  if (!resp.error) {
                    if (resp.status === 'requires_action') {
                      return this.stripe.confirmCardPayment(resp.client_secret).then((result) => {
                        if (result.error) {
                          return $(this.el).find('#errors').html(result.error.message); // the webhook will update db
                        } else {
                          //todo Display error.message in your UI.
                          return this.fire('success',
        resp);
                        }
                      });
              ...
              [
                ...
                [
                  ...
                  [
                    ...
                    'div ' + this._a.user.billing_infos.name,
                    this._a.user.billing_infos.eu_vat ? 'div ' + this._a.user.billing_infos.eu_vat : null,
                    'div ' + this._a.user.billing_infos.address.line1,
                    'div ' + this._a.user.billing_infos.address.postal_code + ' ' + this._a.user.billing_infos.address.city]],
                    [
                      '.sub',
                      [
                        '.title ' + _lang['edit-payment-info']['payment-method'].title,
                        this.create(r.PaymentMethodBloc,
                        {
                          changeable: false
                        }).el
                      ]
                ...
                'td ' + total.toFixed(2) + '€']]]]]],
                ['.bloc.precisions',
                ['div ' + _lang['review-order']['precisions'].replace('%total%',
                total).replace('%period%',
                _lang['review-order'].period[this.selectedPlan.split('-')[1]])]]
              ]
            ]));
          }
        }
      ...
      to: function(id, data) {
        ...
        if (index !== 0) {
          ...
          this['bottom-row'].appendChild(r.toDOM({
            ...
            events: {
              mousedown: () => {
                return this.to(this.steps[index - 1].id);
              }
            }
          }));
        }
        if (step.cta) {
          this['bottom-row'].appendChild(r.toDOM({
            tag: '.cta',
            children: ['.cta-label ' + step.cta.label, '.cta-spinner'], //next
            events: {
    ...
    return r.Seed.extend({
      ...
      tpl: function() {
        return [
          '.payment-method',
          [
            '.card-brand.card-' + (['amex',
            'mastercard',
            'visa'].include(this._a.user.payment_method.card.brand) ? this._a.user.payment_method.card.brand : 'unknown'),
            ['.card-details',
            ['.number_safe •••• •••• •••• ' + this._a.user.payment_method.card.last4,
            '.name-on-card ' + this._a.user.payment_method.name_on_card,
            '.expires_at ' + lang['payment-method']['expires-at'] + ' ' + (this._a.user.payment_method.card.exp_month / 100).toFixed(2).split('.')[1] + '/' + this._a.user.payment_method.card.exp_year]],
            this.changeable ? {
